      ...
      <div className="overflow-x-auto">
        <table className="min-w-[1240px] w-full border-separate border-spacing-0">
          <thead className="bg-[#fbfbf9]">
            <tr>
              {['Avatar', 'Member Code', 'Full Name', 'Email', 'Phone', 'Trainer', 'Membership Status', 'Joined Date', 'Actions'].map((column) => (
              ...
              <tr key={index} className="border-t border-ink/5">
                {Array.from({ length: 9 }).map((__, cellIndex) => (
                  <td key={cellIndex} className="px-4 py-4">
                    <div className="h-4 animate-pulse rounded-full bg-ink/10" />
                  </td>
                ))}
              </tr>
            ))}
