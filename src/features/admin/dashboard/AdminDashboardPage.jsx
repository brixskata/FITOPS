...
export default function AdminDashboardPage() {
  ...
  return <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-7 sm:space-y-8"><SummaryCards summary={dashboard.summary} /><div className="grid gap-6 lg:grid-cols-2"><MembershipStatistics statistics={dashboard.membership_statistics} /><AttendanceOverview statistics={dashboard.attendance_statistics} summary={dashboard.summary} trend={dashboard.trends?.attendance} /></div><RevenueOverview trend={dashboard.trends?.revenue} /><div className="grid gap-6 lg:grid-cols-2"><RecentMembers members={dashboard.recent_members} /><RecentPayments payments={dashboard.recent_payments} /></div><p className="text-right text-xs text-ink/35">Reporting timezone: {dashboard.reporting_timezone || 'Asia/Manila'}</p></motion.div>
}
